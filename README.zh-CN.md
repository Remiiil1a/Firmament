...
* **不是平铺在地面的水**：瀑布侧面、沿坡流下的水现在也按水面绘制，
  ...
  而不是按假定的世界轴。
* **反射**：每个材质的天空反射，加上可选的屏幕空间反射。粗糙表面的反射会在**它自己的
  粗糙度张开的那一圈方向**上取样（两圈、每圈四个方向），而不是只取一个方向当答案；
  它反射的天空也按同一圈取样。金属反射强度、反射所需的光滑度、这一圈铺开多远，
  以及一个"只显示反射"的调试视图都是设置项。
* **辉光（Bloom）**：比显示器能表示的更亮的地方会在周围洒出一圈光晕。
  **表面的高光默认被排除在外**——高光是"一张关于光的画"，而不是光本身。
* **体积雾**：空气是**被步进穿过的介质**，而不是盖在画面上的一层色；按画面的
...
* **水里的日月**：包内带着游戏自己的 `sun.png` 与八张月相，所以水面上的日轮和
  ...
  转也是独立设置项。
* **时间抗锯齿（TAA）**：默认关闭、列为实验性；开启时历史权重随运动变化。
